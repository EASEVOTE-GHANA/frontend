...
const countries = [
  ...
  {"name": "Pitcairn", "dial_code": "+870", "code": "PN", "flag": "🇵🇳"},
...
// Ghana default at the top
const ghanaIndex = countries.findIndex(c => c.code === "GH");
if (ghanaIndex > -1) {
  const gh = countries.splice(ghanaIndex, 1)[0];
  countries.unshift(gh);
}

const lines = countries.map(c => `  { code: "${c.dial_code}", label: "${c.flag} ${c.dial_code}" }`);
console.log(`const COUNTRY_CODES = [\n${lines.join(',\n')}\n];`);
